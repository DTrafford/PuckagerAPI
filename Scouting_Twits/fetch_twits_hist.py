...
sys.stdout = open(
    "../Scouting_Twits/hist/driver_fetch_twits_output_log.txt", "a+")
...
def write_to_db(df, key, last_record):
    try:
        ...
        print('Time stamp : ', dt.datetime.now(),
              '--->  Successfully set index: id in dataframe')
        df_json = json.loads(df.T.to_json()).values()
        print('Time stamp : ', dt.datetime.now(),
              '--->  Successfully converted data to Json format')
        ...
        print('Time stamp : ', dt.datetime.now(),
              '--->  Successfully Updated Mongo DB, collection : ', key)

    except Exception as ex:
        print('Time stamp : ', dt.datetime.now(),
              '---> ', ex)
        print('Time stamp : ', dt.datetime.now(),
              '--->  Unable to save data to Mongo DB')
        df.to_pickle('../DGF_Twits/hist/{}_{}_{}.p'.format(key, last_record['created_at'][0:19].replace(':', '-'),
                                                           last_record['id']))
        print('Time stamp : ', dt.datetime.now(),
              '--->  Data saved locally to "hist" folder')
...
def print_response_limits(resp):
    ...
    print('RateLimit-Reset : ',
          dt.datetime.fromtimestamp(int(resp.headers.get('X-RateLimit-Reset', 0))))
    print("---------------------------------------------------------------------------------------------------------------")
